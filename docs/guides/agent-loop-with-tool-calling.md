...
Model은 자유로운 일반 텍스트가 아니라, Agent가 요구한 결과 형식에 맞춰 최종 답변을
생성합니다.

| Model 응답                                    | Agent의 처리                                               |
| --------------------------------------------- | ---------------------------------------------------------- |
| 정해진 결과 형식을 만족하는 최종 답변         | 해당 답변을 반환하고 실행을 종료합니다.                    |
| Tool 사용 요청                                | Tool을 실행하고 그 결과로 Model의 다음 판단을 요청합니다.  |
| Tool의 입력 또는 실행 오류                    | 오류 결과를 Model에 전달하고 판단을 이어갑니다.            |
| 형식에 맞는 최종 답변도 Tool 사용 요청도 없음 | 정상적으로 처리할 수 없는 응답으로 보고 종료합니다.        |
