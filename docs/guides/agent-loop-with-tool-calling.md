...
주문 목록을 요청할 수 있고, 주문 상태를 확인한 뒤에만 배송 정보가 필요하다고 판단할 수도
있습니다.
...
다음은 사용자가 주문 번호를 모르는 상태에서 배송 상황을 묻는 예시입니다. 실제로 선택되는
...
sequenceDiagram
    ...
    U->>A: 배송 중인 내 주문은 어디까지 왔나요?
    A->>M: 사용자 질문과 사용 가능한 Tool 전달
    M-->>A: get_customer_orders 요청
    A->>TS: 이름과 입력으로 실행 요청
    TS->>T: get_customer_orders 실행
    T-->>A: 고객의 주문 목록과 상태
...
| 상황                                | Agent의 처리                                      |
| ----------------------------------- | ------------------------------------------------- |
| 표시 가능한 최종 텍스트             | 호출자에게 최종 응답을 반환합니다.                |
| Tool Call                           | Tool을 실행하고 Model을 다시 호출합니다.          |
| Tool의 입력 또는 실행 오류          | 오류 결과를 Model에 전달하고 실행을 이어갑니다.   |
| Model API 호출 실패                 | 복구할 수 없는 실행 실패로 종료합니다.            |
| 최종 텍스트와 Tool Call이 모두 없음 | 유효하지 않은 응답으로 실행을 종료합니다.         |
| 다음 Model 호출이 호출 한도를 초과  | Model 호출 한도에 도달한 실행 실패로 종료합니다.  |

현재 한 번의 Agent 실행에서 Model은 최대 5회 호출됩니다. 다섯 번째 Model 응답이 최종
텍스트이면 호출자에게 반환합니다. 다섯 번째 응답이 Tool Call이면 그 결과를 다시 Model에
전달할 수 없으므로 Tool을 실행하기 전에 종료합니다.
